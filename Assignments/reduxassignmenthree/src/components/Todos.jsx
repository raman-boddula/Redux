...
import { getTodoSuccess, getTodoLoading, getTodoError, removeTodoSuccess, removeTodoLoading, removeTodoError, updateTodoSuccess, updateTodoLoading, updateTodoError, addTodoSuccess, addTodoLoading, addTodoError } from "../store/Todos/action";
...
const [text, setText] = React.useState("");
    const { loading, todos, error } = useSelector((state) => ({ loading: state.todoReducer.loading, todos: state.todoReducer.todos, error: state.todoReducer.error }));
    ...
    if (loggedIn)console.log("loggedIn",loggedIn);
    ...
    return loading ? <div>Loading... </div> : error ? <div>something failed</div> : (
        <div>
            <h1><strong> ADD TODO'S LIST </strong>  <Link to="/login">
            <h6>Token - { loggedIn} has loggedIn now</h6>    
        <Button type="primary"
          onClick={() => {
